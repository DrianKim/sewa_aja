...
@section('title', 'Data Detail Kendaraan')
    ...
    <div class="container px-6 py-1 mx-auto">
        <div class="overflow-hidden bg-white rounded-lg shadow-lg">
            ...
            <div
                ...
                <h2 class="text-2xl font-bold text-white">Data Detail Kendaraan</h2>
                        ...
                        @forelse ($details as $detail)
                            <tr class="transition-colors duration-150 hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-sm font-medium text-gray-900">
                                    {{ $detail->kendaraan->merk ?? '-' }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600">
                                    Rp{{ number_format($detail->harga_per_hari, 0, ',', '.') }}
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600">
                                    Rp{{ number_format($detail->harga_per_bulan, 0, ',', '.') }}
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 text-sm font-medium rounded-full
                                        @switch($detail->status)
                                            @case('tersedia') bg-green-100 text-green-700 @break
                                            @case('disewa') bg-yellow-100 text-yellow-700 @break
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form action="{{ route('detail.destroy', $detail->id) }}" method="POST"
                                            ...
                                            data-nama="{{ $detail->kendaraan->merk ?? 'Detail' }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-sm font-medium text-gray-500">Belum ada data detail kendaraan</p>
                                        <p class="mt-1 text-xs text-gray-400">Klik tombol "Tambah Detail" untuk menambahkan
                                            data baru</p>
                                    </div>
                                </td>
                            </tr>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    Swal.fire({
                        title: 'Hapus Data?',
                        html: `Yakin mau hapus detail kendaraan <strong>${nama}</strong>?`,
                        icon: 'warning',
